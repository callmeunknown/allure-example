...
import static com.openmonet.utils.ErrorMessage.UNEXPECTED_VALUE_TYPE;

/* пришлосб написать свою реализацию io.cucumber.core.gherkin.messages.GherkinMessagesStep */
public class CustomStep implements Step {
    ...
    private static Argument extractArgument(List<List<String>> argument, Class argumentType, Messages.Location location) {
        if (!argument.isEmpty()) {
            ...
            throw new IllegalStateException(ErrorMessage.getErrorMessage(UNEXPECTED_VALUE_TYPE, argumentType.getName()));
        }
        return null;
    }
