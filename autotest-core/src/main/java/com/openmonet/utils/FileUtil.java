...
public class FileUtil {
    ...
    public static File searchFileInDirectory(String path, String fileName) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new FileSystemNotFoundException(getErrorMessage(NO_SUCH_FILE, fileName, path));
        }

        if (paths.size() > 1) {
            StringBuilder errorMessage = new StringBuilder();
            errorMessage.append(getErrorMessage(FOUND_MORE_ONE_FILE, fileName))
                    .append(path).append("\\n");
            paths.forEach(p -> errorMessage.append(p.toAbsolutePath()).append("\\n"));
    ...
    public static File getFileByNameOrPath(String path, String fileName) {
        try {
            return searchFileInDirectory(path, fileName);
    ...
    public static String readBodyFromJsonDir(String fileName) {
        return readBodyFromFile(fileName, "resources", "json");
    }
}
